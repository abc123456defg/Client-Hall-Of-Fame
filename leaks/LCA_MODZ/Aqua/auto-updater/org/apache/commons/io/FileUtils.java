...
public class FileUtils
{
    ...
    public static Collection<File> listFiles(final File directory, final IOFileFilter fileFilter, final IOFileFilter dirFilter) {
        try {
            ...
            return visitor.getFileList().stream().map((Function<? super Object, ?>)Path::toFile).collect((Collector<? super Object, ?, Collection<File>>)Collectors.toList());
        }
        catch (IOException e) {
            throw new UncheckedIOException(directory.toString(), e);
        }
    ...
    public static Stream<File> streamFiles(final File directory, final boolean recursive, final String... extensions) throws IOException {
        ...
        return PathUtils.walk(directory.toPath(), filter, toMaxDepth(recursive), false, FileVisitOption.FOLLOW_LINKS).map((Function<? super Path, ? extends File>)Path::toFile);
    }
