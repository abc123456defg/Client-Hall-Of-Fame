...
final class SequenceBuilderIterator<T> extends SequenceScope<T> implements Iterator<T>, Continuation<Unit>, KMappedMarker
{
    ...
    @Nullable
    @Override
    public Object yield(final T value, @NotNull final Continuation<? super Unit> $completion) {
        this.nextValue = value;
        this.state = 3;
        final Continuation c = $completion;
        final int n = 0;
        this.nextStep = (Continuation<? super Unit>)c;
    ...
    @Nullable
    @Override
    public Object yieldAll(@NotNull final Iterator<? extends T> iterator, @NotNull final Continuation<? super Unit> $completion) {
        if (!iterator.hasNext()) {
            return Unit.INSTANCE;
        }
