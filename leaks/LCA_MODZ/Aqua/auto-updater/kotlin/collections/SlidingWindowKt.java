...
public final class SlidingWindowKt
{
    ...
    @NotNull
    public static final <T> Sequence<List<T>> windowedSequence(@NotNull final Sequence<? extends T> $this$windowedSequence, final int size, final int step, final boolean partialWindows, final boolean reuseBuffer) {
        Intrinsics.checkNotNullParameter($this$windowedSequence, "<this>");
        checkWindowSizeStep(size, step);
        return (Sequence<List<T>>)new Sequence<List<? extends T>>($this$windowedSequence, size, step, partialWindows, reuseBuffer) {
            @NotNull
            @Override
    ...
    public static final <T> Iterator<List<T>> windowedIterator(@NotNull final Iterator<? extends T> iterator, final int size, final int step, final boolean partialWindows, final boolean reuseBuffer) {
        ...
        return SequencesKt__SequenceBuilderKt.iterator((Function2<? super SequenceScope<? super List<T>>, ? super Continuation<? super Unit>, ?>)new SlidingWindowKt$windowedIterator.SlidingWindowKt$windowedIterator$1(size, step, (Iterator)iterator, reuseBuffer, partialWindows, (Continuation)null));
    }
}
