    ...
    protected void registerPackets() {
        ...
        ((AbstractProtocol<ClientboundPackets1_19_1, C2, S1, S2>)this).registerClientbound(ClientboundPackets1_19_1.JOIN_GAME, new PacketRemapper() {
            ...
            public void registerMap() {
                ...
                this.map(Type.STRING);
                final ChatRegistryStorage chatTypeStorage;
                final CompoundTag registry;
                final ListTag chatTypes;
                final Iterator<Tag> iterator;
                Tag chatType;
                CompoundTag chatTypeCompound;
                NumberTag idTag;
                this.handler(wrapper -> {
                    chatTypeStorage = wrapper.user().get(ChatRegistryStorage.class);
                    chatTypeStorage.clear();
                    registry = wrapper.get(Type.NBT, 0);
                    chatTypes = registry.get("minecraft:chat_type").get("value");
                    chatTypes.iterator();
                    while (iterator.hasNext()) {
                        chatType = iterator.next();
        ...
        ((Protocol<ClientboundPackets1_19_1, ClientboundPackets1_19, S1, S2>)this).registerClientbound(ClientboundPackets1_19_1.PLAYER_CHAT, ClientboundPackets1_19.SYSTEM_CHAT, new PacketRemapper() {
            @Override
            public void registerMap() {
                final PlayerMessageSignature signature;
                ReceivedMessagesStorage messagesStorage;
                PacketWrapper chatAckPacket;
                final String plainMessage;
                JsonElement message;
                final JsonElement decoratedMessage;
                final JsonElement unsignedMessage;
                final int filterMaskType;
                final int chatTypeId;
                final JsonElement senderName;
                final JsonElement targetName;
                final JsonElement decoratedMessage2;
                this.handler(wrapper -> {
                    if (wrapper.read((Type<Boolean>)Type.BOOLEAN)) {
                        wrapper.read(Type.BYTE_ARRAY_PRIMITIVE);
                    }
                    signature = wrapper.read(Type.PLAYER_MESSAGE_SIGNATURE);
                    if (!signature.uuid().equals(Protocol1_19To1_19_1.ZERO_UUID) && signature.signatureBytes().length != 0) {
                        messagesStorage = wrapper.user().get(ReceivedMessagesStorage.class);
                        messagesStorage.add(signature);
                        if (messagesStorage.tickUnacknowledged() > 64) {
                            messagesStorage.resetUnacknowledgedCount();
                            chatAckPacket = wrapper.create(ServerboundPackets1_19_1.CHAT_ACK);
                    ...
                    plainMessage = wrapper.read(Type.STRING);
                    message = null;
                    decoratedMessage = wrapper.read(Type.OPTIONAL_COMPONENT);
                    ...
                    unsignedMessage = wrapper.read(Type.OPTIONAL_COMPONENT);
                    ...
                    filterMaskType = wrapper.read((Type<Integer>)Type.VAR_INT);
                    if (filterMaskType == 2) {
                        wrapper.read(Type.LONG_ARRAY_PRIMITIVE);
                    }
                    chatTypeId = wrapper.read((Type<Integer>)Type.VAR_INT);
                    senderName = wrapper.read(Type.COMPONENT);
                    targetName = wrapper.read(Type.OPTIONAL_COMPONENT);
                    decoratedMessage2 = Protocol1_19To1_19_1.this.decorateChatMessage(wrapper, chatTypeId, senderName, targetName, message);
        ...
        ((AbstractProtocol<ClientboundPackets1_19_1, C2, S1, S2>)this).registerClientbound(ClientboundPackets1_19_1.SYSTEM_CHAT, new PacketRemapper() {
            ...
            public void registerMap() {
                ...
                this.handler(wrapper -> {
                    content = wrapper.passthrough(Type.COMPONENT);
                    Protocol1_19To1_19_1.this.translatableRewriter.processText(content);
                    overlay = wrapper.read((Type<Boolean>)Type.BOOLEAN);
        ...
        ((AbstractProtocol<C1, C2, S1, ServerboundPackets1_19>)this).registerServerbound(ServerboundPackets1_19.CHAT_MESSAGE, new PacketRemapper() {
            ...
            public void registerMap() {
                ...
                this.map(Type.BOOLEAN);
                final ReceivedMessagesStorage messagesStorage;
                this.handler(wrapper -> {
                    messagesStorage = wrapper.user().get(ReceivedMessagesStorage.class);
                    messagesStorage.resetUnacknowledgedCount();
        ...
        ((AbstractProtocol<C1, C2, S1, ServerboundPackets1_19>)this).registerServerbound(ServerboundPackets1_19.CHAT_COMMAND, new PacketRemapper() {
            ...
            public void registerMap() {
                ...
                final ReceivedMessagesStorage messagesStorage;
                this.handler(wrapper -> {
                    for (signatures = wrapper.passthrough((Type<Integer>)Type.VAR_INT), i = 0; i < signatures; ++i) {
        ...
        this.registerServerbound(State.LOGIN, ServerboundLoginPackets.ENCRYPTION_KEY.getId(), ServerboundLoginPackets.ENCRYPTION_KEY.getId(), new PacketRemapper() {
            ...
            public void registerMap() {
                ...
                final boolean isNonce;
                this.handler(wrapper -> {
                    nonceStorage = wrapper.user().remove(NonceStorage.class);
                    isNonce = wrapper.read((Type<Boolean>)Type.BOOLEAN);
        ...
        this.registerClientbound(State.LOGIN, ClientboundLoginPackets.CUSTOM_QUERY.getId(), ClientboundLoginPackets.CUSTOM_QUERY.getId(), new PacketRemapper() {
            ...
            public void registerMap() {
                ...
                byte[] data2;
                this.handler(wrapper -> {
                    identifier = wrapper.get(Type.STRING, 0);
                    if (identifier.equals("velocity:player_info")) {
                        data = wrapper.passthrough(Type.REMAINING_BYTES);
