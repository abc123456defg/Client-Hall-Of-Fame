...
public class DumpSubCmd extends ViaSubCommand
{
    ...
    public boolean execute(final ViaCommandSender sender, final String[] args) {
        final VersionInfo version = new VersionInfo(System.getProperty("java.version"), System.getProperty("os.name"), Via.getAPI().getServerVersion().lowestSupportedVersion(), Via.getManager().getProtocolManager().getSupportedVersions(), Via.getPlatform().getPlatformName(), Via.getPlatform().getPlatformVersion(), Via.getPlatform().getPluginVersion(), "git-ViaVersion-4.4.2-SNAPSHOT:08727f9", Via.getManager().getSubPlatforms());
