...
public abstract class AbstractViaConfig extends Config implements ViaVersionConfig
{
    ...
    protected void loadFields() {
        ...
        final Map<String, String> worlds = this.get("map-1_16-world-names", (Class<HashMap<String, String>>)Map.class, new HashMap<String, String>());
        ...
        this.chatTypeFormats = this.get("chat-types-1_19", (Class<HashMap<String, String>>)Map.class, new HashMap<String, String>());
    }
    
    private BlockedProtocolVersions loadBlockedProtocolVersions() {
        final List<Integer> blockProtocols = this.getListSafe("block-protocols", Integer.class, "Invalid blocked version protocol found in config: '%s'");
        final List<String> blockVersions = this.getListSafe("block-versions", String.class, "Invalid blocked version found in config: '%s'");
        final IntSet blockedProtocols = new IntOpenHashSet(blockProtocols);
        int lowerBound = -1;
        int upperBound = -1;
        for (final String s : blockVersions) {
            if (s.isEmpty()) {
                continue;
            }
            ...
            if (c == '<' || c == '>') {
                ...
                final ProtocolVersion protocolVersion = this.protocolVersion(s);
                if (protocolVersion == null) {
                    continue;
                }
        ...
        if (lowerBound != -1 || upperBound != -1) {
            ...
            blockedProtocols.removeIf(version -> {
                if ((n != -1 && version < n) || (n2 != -1 && version > n2)) {
                    protocolVersion2 = ProtocolVersion.getProtocol(version);
                    Via.getPlatform().getLogger().warning("Blocked protocol version " + protocolVersion2.getName() + "/" + protocolVersion2.getVersion() + " already covered by upper or lower bound");
                    return true;
                }
                else {
                    return false;
    ...
    private ProtocolVersion protocolVersion(final String s) {
        final ProtocolVersion protocolVersion = ProtocolVersion.getClosest(s);
        if (protocolVersion == null) {
