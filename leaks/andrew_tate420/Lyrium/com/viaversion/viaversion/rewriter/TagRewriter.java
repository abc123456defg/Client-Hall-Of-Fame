...
public class TagRewriter
{
    ...
    public PacketHandler getGenericHandler() {
        ...
        RegistryType type;
        return wrapper -> {
            for (length = (editedLength = wrapper.passthrough((Type<Integer>)Type.VAR_INT)), i = 0; i < length; ++i) {
                registryKey = wrapper.read(Type.STRING);
                if (this.toRemove.contains(registryKey)) {
                    wrapper.set(Type.VAR_INT, 0, --editedLength);
                    for (tagsSize = wrapper.read((Type<Integer>)Type.VAR_INT), j = 0; j < tagsSize; ++j) {
