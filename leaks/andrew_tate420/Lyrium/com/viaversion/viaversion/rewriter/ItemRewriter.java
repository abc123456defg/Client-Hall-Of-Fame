    ...
    public void registerWindowPropertyEnchantmentHandler(final ClientboundPacketType packetType) {
        this.protocol.registerClientbound(packetType, new PacketRemapper() {
            ...
            public void registerMap() {
                ...
                short enchantmentId;
                this.handler(wrapper -> {
                    property = wrapper.passthrough((Type<Short>)Type.SHORT);
                    if (property >= 4 && property <= 6) {
                        mappings = ItemRewriter.this.protocol.getMappingData().getEnchantmentMappings();
                        enchantmentId = (short)mappings.getNewId(wrapper.read((Type<Short>)Type.SHORT));
