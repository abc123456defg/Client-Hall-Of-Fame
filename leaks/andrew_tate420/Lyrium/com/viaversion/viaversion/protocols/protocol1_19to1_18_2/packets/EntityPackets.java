...
public final class EntityPackets extends EntityRewriter<Protocol1_19To1_18_2>
{
    ...
    public void registerPackets() {
        ...
        ((AbstractProtocol<ClientboundPackets1_18, C2, S1, S2>)this.protocol).registerClientbound(ClientboundPackets1_18.SPAWN_ENTITY, new PacketRemapper() {
            ...
            public void registerMap() {
                ...
                this.handler(wrapper -> {
                    ...
                    entityType = ((com.viaversion.viaversion.api.rewriter.EntityRewriter<Protocol>)EntityPackets.this).tracker(wrapper.user()).entityType(entityId);
        ...
        ((Protocol<ClientboundPackets1_18, ClientboundPackets1_19, S1, S2>)this.protocol).registerClientbound(ClientboundPackets1_18.SPAWN_PAINTING, ClientboundPackets1_19.SPAWN_ENTITY, new PacketRemapper() {
            @Override
            public void registerMap() {
                this.map(Type.VAR_INT);
                this.map(Type.UUID);
                ...
                final PacketWrapper metaPacket;
                final ArrayList<Metadata> metadata;
                this.handler(wrapper -> {
                    wrapper.write(Type.VAR_INT, Entity1_19Types.PAINTING.getId());
                    motive = wrapper.read((Type<Integer>)Type.VAR_INT);
                    blockPosition = wrapper.read(Type.POSITION1_14);
                    direction = wrapper.read((Type<Byte>)Type.BYTE);
                    ...
                    wrapper.cancel();
                    metaPacket = wrapper.create(ClientboundPackets1_19.ENTITY_METADATA);
        ...
        ((Protocol<ClientboundPackets1_18, ClientboundPackets1_19, S1, S2>)this.protocol).registerClientbound(ClientboundPackets1_18.SPAWN_MOB, ClientboundPackets1_19.SPAWN_ENTITY, new PacketRemapper() {
            @Override
            public void registerMap() {
                this.map(Type.VAR_INT);
                this.map(Type.UUID);
                ...
                final byte pitch;
                this.handler(wrapper -> {
                    yaw = wrapper.read((Type<Byte>)Type.BYTE);
                    pitch = wrapper.read((Type<Byte>)Type.BYTE);
        ...
        ((AbstractProtocol<ClientboundPackets1_18, C2, S1, S2>)this.protocol).registerClientbound(ClientboundPackets1_18.JOIN_GAME, new PacketRemapper() {
            ...
            public void registerMap() {
                ...
                final HashMap<String, DimensionData> dimensionDataMap;
                final HashMap<CompoundTag, String> dimensionsMap;
                final Iterator<Tag> iterator;
                Tag dimension;
                CompoundTag dimensionCompound;
                CompoundTag element;
                String name;
                final DimensionRegistryStorage registryStorage;
                this.handler(wrapper -> {
                    tag = wrapper.get(Type.NBT, 0);
                    tag.put("minecraft:chat_type", EntityPackets.CHAT_REGISTRY.clone());
                    dimensions = tag.get("minecraft:dimension_type").get("value");
                    dimensionDataMap = new HashMap<String, DimensionData>(dimensions.size());
                    dimensionsMap = new HashMap<CompoundTag, String>(dimensions.size());
                    dimensions.iterator();
                    while (iterator.hasNext()) {
                        dimension = iterator.next();
                        dimensionCompound = (CompoundTag)dimension;
                        element = dimensionCompound.get("element");
                    ...
                    registryStorage = wrapper.user().get(DimensionRegistryStorage.class);
                ...
                final PacketWrapper displayPreviewPacket;
                this.handler(wrapper -> {
                    displayPreviewPacket = wrapper.create(ClientboundPackets1_19.SET_DISPLAY_CHAT_PREVIEW);
    ...
    private static void writeDimensionKey(final PacketWrapper wrapper, final DimensionRegistryStorage registryStorage) throws Exception {
        ...
        if (dimensionKey == null) {
            ...
            dimensionKey = registryStorage.dimensions().entrySet().stream().map(it -> new Pair(it, Maps.difference((Map<?, ?>)currentDimension.getValue(), (Map<?, ?>)it.getKey().getValue()).entriesInCommon())).filter(it -> it.value().containsKey("min_y") && it.value().containsKey("height")).max(Comparator.comparingInt(it -> it.value().size())).orElseThrow(() -> {
                new IllegalArgumentException("Dimension not found in registry data from join packet: " + obj);
                return o;
            }).key().getValue();
        }
