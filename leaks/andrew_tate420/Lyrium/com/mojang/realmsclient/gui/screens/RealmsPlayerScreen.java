...
public class RealmsPlayerScreen extends RealmsScreen
{
    ...
    public void buttonClicked(final RealmsButton button) {
        ...
        switch (button.id()) {
            ...
            case 1: {
                Realms.setScreen((RealmsScreen)new RealmsInviteScreen(this.lastScreen, this, this.serverData));
                break;
            }
            case 3: {
                Realms.setScreen((RealmsScreen)new RealmsActivityScreen(this, this.serverData));
                break;
            }
            default: {}
        }
    ...
    private void backButtonClicked() {
        if (this.stateChanged) {
            Realms.setScreen((RealmsScreen)this.lastScreen.getNewScreen());
        }
        else {
    ...
    private void uninvite(final int index) {
        if (index >= 0 && index < this.serverData.players.size()) {
            ...
            final RealmsConfirmScreen confirmScreen = new RealmsConfirmScreen(this, "Question", getLocalizedString("mco.configure.world.uninvite.question") + " '" + playerInfo.getName() + "' ?", 2);
    ...
    public void render(final int xm, final int ym, final float a) {
        ...
        t.vertex(0.0, (double)this.height(), 0.0).tex(0.0, (double)((this.height() - bottom_border) / 32.0f + 0.0f)).color(64, 64, 64, 255).endVertex();
        t.vertex((double)this.width(), (double)this.height(), 0.0).tex((double)(this.width() / 32.0f), (double)((this.height() - bottom_border) / 32.0f + 0.0f)).color(64, 64, 64, 255).endVertex();
        t.vertex((double)this.width(), (double)bottom_border, 0.0).tex((double)(this.width() / 32.0f), 0.0).color(64, 64, 64, 255).endVertex();
