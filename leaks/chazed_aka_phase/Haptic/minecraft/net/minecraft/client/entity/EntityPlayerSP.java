...
import haptic.event.impl.PositionType;
...
public class EntityPlayerSP extends AbstractClientPlayer
{
     ...
     * to be re-trasmitted
     */
    public double lastReportedPosX; // was private

    /**
     * The last Y position which was transmitted to the server, used to determine when the Y position changes and needs
     * to be re-transmitted
     */
    public double lastReportedPosY; // was private

    /**
     * The last Z position which was transmitted to the server, used to determine when the Z position changes and needs
     * to be re-transmitted
     */
    public double lastReportedPosZ; // was private

    /**
     * The last yaw value which was transmitted to the server, used to determine when the yaw changes and needs to be
     * re-transmitted
     */
    public float lastReportedYaw; // was private

    /**
     * The last pitch value which was transmitted to the server, used to determine when the pitch changes and needs to
     * be re-transmitted
     */
    public float lastReportedPitch; // was private

    /** the last sneaking state sent to the server */
    public boolean serverSneakState; // was private

    /** the last sprinting state sent to the server */
    public boolean serverSprintState; // was private

    /**
     * Reset to 0 every time position is sent to the server, used to send periodic updates every 20 ticks even when the
     * player is not moving.
     */
    public int positionUpdateTicks; // was private
    ...
    public void onUpdateWalkingPlayer()
    {
        ...
        EventMotionUpdate event = new EventMotionUpdate(mc.thePlayer.posX, mc.thePlayer.posY, mc.thePlayer.posZ, mc.thePlayer.rotationYaw, mc.thePlayer.rotationPitch, mc.thePlayer.onGround);
        event.call();
        ...
        if (this.isCurrentViewEntity())
        {
            ...
            {
            	if(event.getPositionType() != null) {
            		if(event.getPositionType() == PositionType.C03) {
            			this.sendQueue.addToSendQueue(new C03PacketPlayer(event.isOnGround()));
            		} else if(event.getPositionType() == PositionType.C04) {
    ...
    public void onLivingUpdate()
    {
        ...
        {
        	EventSlowdown event = new EventSlowdown(0.2F, 0.2F);
        	event.call();
        	if(!event.isCancelled()) {
